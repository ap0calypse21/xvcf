...
static bool HWIDBan(DWORD dwAID, int iBanDuration)
{
    if (!dwAID)
        return false;
    ...
    if (iBanDuration == 0)
    {
        snprintf(szQuery, sizeof(szQuery), "UPDATE `account`.`hwid` SET `ban` = 0, `ban_until` = NULL WHERE `account_id` = %u", dwAID);
    }
    else
    {
        time_t tBanUntil = time(0) + iBanDuration;
		snprintf(szQuery, sizeof(szQuery), "UPDATE `account`.`hwid` SET `ban` = 1, `ban_until` = FROM_UNIXTIME(%lld) WHERE `account_id` = %u", (long long)tBanUntil, dwAID);
...
ACMD(do_hwid_ban)
{
    ...
    if (!*arg1 || !*arg2)
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "Usage: hwid_ban <player name> <ban duration (hours)>");
        return;
    }

    DWORD dwAID = 0;
    const char* c_szName = arg1;
    const char* c_szDuration = arg2;

    int iBanDuration = 0; // Initialize ban duration
    if (strcmp(c_szDuration, "0") != 0)
    {
        str_to_number(iBanDuration, c_szDuration);
        iBanDuration *= 3600; // Convert hours to seconds
    }

    LPCHARACTER pkCh = CHARACTER_MANAGER::instance().FindPC(c_szName);
    if (pkCh != nullptr) // Ensure pkCh is not null
    {
        ...
        if (HWIDBan(dwAID, iBanDuration))
        {
            if (iBanDuration == 0)
            {
                ch->ChatPacket(CHAT_TYPE_INFO, "Ban removed for player %s (ID: %u)", c_szName, dwAID);
                sys_log(0, "%s removed (hwid_ban) player %s (ID: %u)", ch->GetName(), c_szName, dwAID);
            }
            else
            {
                pkCh->Disconnect("Banned");
                ch->ChatPacket(CHAT_TYPE_INFO, "Banned player %s (ID: %u) for %s hours", c_szName, dwAID, c_szDuration);
                sys_log(0, "%s banned (hwid_ban) player %s (ID: %u) for %s hours", ch->GetName(), c_szName, dwAID, c_szDuration);
            }
        }
        else
        {
            sys_log(0, "Failed to ban player %s (ID: %u)", c_szName, dwAID);
        }
    ...
    else
    {
        ...
        if (pkCCI != nullptr) // Ensure pkCCI is not null
        {
            LPDESC pkDesc = pkCCI->pkDesc;
            if (pkDesc != nullptr && pkDesc->GetCharacter() != nullptr) // Ensure pkDesc and pkDesc->GetCharacter() are not null
            {
                ...
                if (HWIDBan(dwAID, iBanDuration))
                {
                    ...
                    {
                        pkDesc->GetCharacter()->Disconnect("Banned");
                        ch->ChatPacket(CHAT_TYPE_INFO, "Banned player %s (ID: %u) for %s hours", c_szName, dwAID, c_szDuration);
                        sys_log(0, "%s banned (hwid_ban) player %s (ID: %u) for %s hours", ch->GetName(), c_szName, dwAID, c_szDuration);
                    }
                }
                ...
                {
                    sys_log(0, "Failed to ban player %s (ID: %u) from peer.", c_szName, dwAID);
                }
        ...
        else
        {
            ...
            if (pMsg->Get()->uiNumRows > 0)
            {
                ...
                if (dwAID != 0)
                {
                    ...
                    {
                        sys_log(0, "Failed to ban player %s (ID: %u) from direct query.", c_szName, dwAID);
                    }
                    return;
                }
            }
            ...
            {
                ch->ChatPacket(CHAT_TYPE_INFO, "Unknown player.");
                return;
            }
        }
    }
...
ACMD(do_warp)
{
	...
	int x = 0, y = 0;
#if defined(ENABLE_CMD_WARP_IN_DUNGEON) || defined(ENABLE_DUNGEON_TURN_BACK)
	int mapIndex = 0;
#endif
	if (isnhdigit(*arg1) && isnhdigit(*arg2))
	{
...
#if defined(ENABLE_BONUS_METINSTONE) || defined(ENABLE_BONUS_BOSS)
...
#endif
#if defined(ENABLE_BONUS_METINSTONE)
, tch->GetPoint(POINT_ATTBONUS_METINSTONE)
#endif
#if defined(ENABLE_BONUS_BOSS)
, tch->GetPoint(POINT_ATTBONUS_BOSS)
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "   CZ:%3d%%", tch->GetPoint(POINT_ATTBONUS_CZ));
#endif
...
void SendLocaleNotice(bool bBigFont, const char* c_pszNotice, ...)
{
...
void SendNoticeMap(bool bBigFont, int iMapIndex, const char* c_pszNotice, ...)
{
	const DESC_MANAGER::DESC_SET& c_ref_set = DESC_MANAGER::instance().GetClientSet();
...
ACMD(do_map_notice)
{
	SendNoticeMap(false, ch->GetMapIndex(), argument);
}
...
ACMD(do_book)
{
	...
	LPITEM item = ch->AutoGiveItem(50300, 1, false, false);
...
// #endif
		...
		default:
			...
			{
				SendNoticeMap(false, ch->GetMapIndex(), "Reloading regens!");
				regen_free_map(ch->GetMapIndex());
				CHARACTER_MANAGER::instance().DestroyCharacterInMap(ch->GetMapIndex());
				regen_reload(ch->GetMapIndex());
				SendNoticeMap(false, ch->GetMapIndex(), "Regens reloaded!");
			}
			break;
		}
	}
...
#ifdef ENABLE_COMMAND_FLUSH_FIX
	DWORD pid = 0;
...
ACMD(do_battlepass_get_info) {
	if (CBattlePassManager::instance().GetNormalBattlePassID() == 0) {
		...
		std::unique_ptr<SQLMsg> pMsgRegistred(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM `player`.`battlepass_playerindex` WHERE "
																				"battlepass_type=1 AND battlepass_id=%d"
																				, CBattlePassManager::instance().GetNormalBattlePassID()));
		std::unique_ptr<SQLMsg> pMsgCompledet(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM `player`.`battlepass_playerindex` WHERE "
																				"battlepass_type=1 AND battlepass_id=%d AND battlepass_completed=1"
																				, CBattlePassManager::instance().GetNormalBattlePassID()));
	...
	if (CBattlePassManager::instance().GetPremiumBattlePassID() == 0) {
		...
		std::unique_ptr<SQLMsg> pMsgRegistred(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM `player`.`battlepass_playerindex` WHERE "
																				"`battlepass_type`=2 AND `battlepass_id`=%d"
																				, CBattlePassManager::instance().GetPremiumBattlePassID()));
		std::unique_ptr<SQLMsg> pMsgCompledet(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM `player`.`battlepass_playerindex` WHERE "
																				"`battlepass_type`=2 AND `battlepass_id`=%d AND `battlepass_completed`=1"
																				, CBattlePassManager::instance().GetPremiumBattlePassID()));
	...
	if (CBattlePassManager::instance().GetEventBattlePassID() == 0) {
		...
		std::unique_ptr<SQLMsg> pMsgRegistred(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM `player`.`battlepass_playerindex` WHERE "
																				"`battlepass_type`=3 AND `battlepass_id`=%d"
																				, CBattlePassManager::instance().GetEventBattlePassID()));
		std::unique_ptr<SQLMsg> pMsgCompledet(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM `player`.`battlepass_playerindex` WHERE "
																				"`battlepass_type`=3 AND `battlepass_id`=%d AND `battlepass_completed`=1"
																				, CBattlePassManager::instance().GetEventBattlePassID()));
...
ACMD(do_battlepass_set_mission) {
	...
	int32_t battlepass_type, mission_index, value;
	...
	if (battlepass_type == 1 && CBattlePassManager::instance().GetNormalBattlePassID() == 0) {
		ch->ChatPacket(CHAT_TYPE_INFO, "No normal Battlepass is currently active");
		return;
	} else if (battlepass_type == 2 && CBattlePassManager::instance().GetPremiumBattlePassID() == 0) {
		ch->ChatPacket(CHAT_TYPE_INFO, "No premium Battlepass is currently active");
		return;
	} else if (battlepass_type == 3 && CBattlePassManager::instance().GetEventBattlePassID() == 0) {
		ch->ChatPacket(CHAT_TYPE_INFO, "No event Battlepass is currently active");
		return;
	}
	...
	if (battlepass_type == 2 && CBattlePassManager::instance().GetPremiumBattlePassID() != (uint32_t)(tch->GetExtBattlePassPremiumID())) {
